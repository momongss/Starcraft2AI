...
def state_processing(obs):
  ...
  t = {}
  for d in input_data:
    if sys.getsizeof(d) not in t.keys():
      t[sys.getsizeof(d)] = 1
    else:
      t[sys.getsizeof(d)] += 1
...
def run_loop(agents, env, max_frames=0, max_episodes=0):
  ...
  try:
    ...
    dir_path = "C:\\Users\\rkrp1\\Desktop\\종합설계\\pysc2-DQN\\network_store\\"
    episode_count_file = dir_path + "episode_count.txt"
    ...
    while not max_episodes or total_episodes < max_episodes:
      ...
      # 폴더 만들기
      try:
        file_path = dir_path + str(total_episodes)
        print(file_path)
      ...
      while True:
        ...
        args = actions[0][2]
        actions = [actions[0][0]]
        print("fucntion_id", function_id, "args", args)
        ...
        if total_frames and total_frames % 10 == 0:
          dataframe = pd.DataFrame(replay_buffer)
          dataframe.to_csv(file_path + "\\" + str(replay_store_count) + ".csv", header=False, index=False)
          replay_store_count += 1
          replay_buffer = deque(maxlen=10)
