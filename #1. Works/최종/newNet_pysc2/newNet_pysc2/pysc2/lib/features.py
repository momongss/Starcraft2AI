...
def features_from_game_info(game_info, agent_interface_format=None,
                            map_name=None, useRGB=True, **kwargs):
  ...
  if useRGB and game_info.options.HasField("render"):
...
class Features(object):
  ...
  def custom_transform_obs(self, obs):
    ...
    if not self._raw:
      ...
      for i in self.available_actions(obs.observation):
        out["available_actions"][i] = 1
