...
def core(embedded_scalar, embedded_entity, embedded_spatial):
    # embedded_scalar = layers.Input(shape=(512,), dtype='float32')
    ...
    LSTM_input = layers.Reshape((1, 1024))(LSTM_input)
    ...
    # x = tf.keras.layers.CuDNNLSTM(384, return_sequences=True, stateful=True)(LSTM_input)
    # x = tf.keras.layers.CuDNNLSTM(384, return_sequences=True, stateful=True)(x)
    # lstm_out = tf.keras.layers.CuDNNLSTM(384, stateful=True)(x)

    core_model = models.Model(inputs=[embedded_scalar, embedded_entity, embedded_spatial],
                              outputs={'lstm_out': lstm_out})
    return core_model
