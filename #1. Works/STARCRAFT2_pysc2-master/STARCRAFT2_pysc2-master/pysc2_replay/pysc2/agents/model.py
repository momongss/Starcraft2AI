...
class DQN:
    ...
    def build_network(self, width=10, L_rate=1e-1):
        ...
        self.W1 = tf.Variable(tf.random_uniform([self.input_size, width], 0, 0.01), dtype=tf.float32)
        self.W2 = tf.Variable(tf.random_uniform([width, width], 0, 0.01), dtype=tf.float32)
        self.W3 = tf.Variable(tf.random_uniform([width, width], 0, 0.01), dtype=tf.float32)
        self.W4 = tf.Variable(tf.random_uniform([width, width], 0, 0.01), dtype=tf.float32)
        self.W5 = tf.Variable(tf.random_uniform([width, self.output_size], 0, 0.01), dtype=tf.float32)

        self.L1 = tf.nn.tanh(tf.matmul(self.x, self.W1))
        self.L2 = tf.nn.tanh(tf.matmul(self.L1, self.W2))
        self.L3 = tf.nn.tanh(tf.matmul(self.L2, self.W3))
        self.L4 = tf.nn.tanh(tf.matmul(self.L3, self.W4))
        self.Q_pre = tf.matmul(self.L4, self.W5)
