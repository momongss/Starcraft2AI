...
def run_loop(agents, env, max_frames=0, max_episodes=0):
  ...
  with open("C:\\Users\\rkrp1\\Desktop\\종합설계\\pysc2-master-excute\\pysc2-master\\_replay_data\\episode_count.txt", 'r') as f:
    previous_episode = int(f.read())

  total_frames = 0
  total_episodes = previous_episode
  ...
  def save_replay_buffer(replay_buffer, eplisode_name):
    with open("C:\\Users\\rkrp1\\Desktop\\종합설계\\pysc2-master-excute\\pysc2-master\\_replay_data\\"+eplisode_name+".txt", 'wb') as f:
  ...
  try:
    ...
    while not max_episodes or total_episodes < max_episodes:
      total_episodes += 1
      with open("C:\\Users\\rkrp1\\Desktop\\종합설계\\pysc2-master-excute\\pysc2-master\\_replay_data\\episode_count.txt", 'w') as f:
      ...
      while True:
        ...
        if total_frames % 20 == 0:
          save_replay_buffer(replay_buffer, str(total_episodes))
          replay_buffer = []

        cur_time = time.time()
        if cur_time - pre_time > 1:
          print(total_frames)
        while cur_time - pre_time > 1:
          pre_time = pre_time + 1
          print('1sec')
        #
        # # 10 번의 스탭마다 미니배치로 학습
