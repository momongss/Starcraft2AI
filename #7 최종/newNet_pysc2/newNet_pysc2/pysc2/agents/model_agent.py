...
alphastar_logs = keras.callbacks.TensorBoard('.\\tensorboard_logs\\alphastar_logs',
                                                   update_freq=1)
...
class ModelAgent(base_agent.BaseAgent):
    def model_update_play(self, action, arg, obs):
        ...
        if self.update_count == LRChangePoint:
            ...
            sys.exit()
            # self.startcraft_model.compile(optimizer=keras.optimizers.Adam(learning_rate=1e-3), loss=keras.losses.KLDivergence())
            # print('Learning Rate 변경됨')
        ...
        for key in total_obs:
            obs[key] = normalize(np.asarray(obs[key].reshape(reshape_map[key])))
    ...
    def model_predict(self, obs, available_action):
        ...
        for key in total_obs:
            obs[key] = np.asarray(obs[key].reshape(reshape_map[key])) / 10000.0
