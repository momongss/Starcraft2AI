...
flags.DEFINE_string("replay", "C:\\Users\\rkrp1\\Desktop\\종합설계\\#1. Works\\REPLAY\\Simple64_2020-04-29-09-29-40.SC2Replay", "Name of a replay to show.")
...
def reverse_action(action):
    ...
    def func_call_ability(ability_id, cmd_type, *args):
        ...
        for func in actions.ABILITY_IDS[ability_id]:
            if func.function_type is cmd_type or ability_id == 1:
  ...
  with run_config.start(
    ...
    if FLAGS.map:
      ...
      if info.player_info[0].player_info.race_actual != 1 or info.map_name != "Abyssal Reef LE" or info.player_info[0].player_mmr < 1000:
          print("Race not Match", info.player_info[0].player_info.race_actual, info.player_info[1].player_info.race_actual, info.map_name)
          return
    ...
    if FLAGS.render:
      ...
      try:
        ...
        while True:
          ...
          if controller.status.name != 'in_replay':
              break
          f = features.features_from_game_info(game_info=controller.game_info(), useRGB=False)
          ...
          try:
              agent.update(agent_obs, func, args)
          except:
              print("not match", func, args)
          if save_step == 1000:
              saver.save(agent.sess, 'weight/DQN-model')
              print("weight 저장됨")
              save_step = 0
          save_step += 1
