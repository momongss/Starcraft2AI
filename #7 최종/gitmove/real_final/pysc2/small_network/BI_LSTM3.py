...
def small_action_model():
    ...
    x = layers.Bidirectional(layers.LSTM(573, return_sequences=True))(entity_list)
    x = layers.Bidirectional(layers.LSTM(573, return_sequences=True))(x)
    lstm_out = layers.Bidirectional(layers.LSTM(573), name='lstm_out')(x)

    # Dense
    action_type_logits = layers.Dense(573, name='action_type_logits')(lstm_out)
