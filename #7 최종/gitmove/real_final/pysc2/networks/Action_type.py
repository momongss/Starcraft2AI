...
def action_type_encoder(lstm_output, scalar_context):
    ...
    x = layers.Dense(conv_width, activation='sigmoid')(scalar_context)
    x = layers.BatchNormalization()(x)
    gate = layers.Activation('sigmoid')(x)
    gate_input = layers.multiply([Resblock_flatten, gate])
    # action_type_logits = layers.Dense(action_type_len, name='action_type_logits')(gate_input)
    action_type_logits = layers.Dense(action_type_len, name='action_type_logits')(gate_input)
    # x = layers.BatchNormalization()(x)
    ...
    x = layers.Dense(256)(scalar_context)
    x = layers.BatchNormalization()(x)
    gate = layers.Activation('sigmoid')(x)
